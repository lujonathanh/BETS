...
def plot_coef(df, cause_gene, effect_gene, lag, coef, savefile=True, file_prefix="", title=None,
         cause_label=None, effect_label=None, legend_fontsize=None, ylabel=None, is_ensg=False, **kwargs):
    if title == None:
        title = "Cause-Effect pair: (" + cause_gene + ", " + effect_gene + ")"

    if savefile == True:
        filename = file_prefix + cause_gene.replace(".", ",") + "-" + effect_gene.replace(".", ",") + "-lag-" + str(int(lag)) + "-coef-" + float_to_label(coef)
