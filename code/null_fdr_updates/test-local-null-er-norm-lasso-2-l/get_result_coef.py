...
def load_and_run(args):
    ...
    for fdr in fdrs:
        ...
        fdr_prefix = "fdr-" + str(fdr) + os.sep + save_prefix + "-" + stratify_by
        ...
        for i in range(acoefs.shape[0]):
            ...
            fdr_matr_filename = fdr_prefix + "-" + str(i+1) + "-fdr-" + str(fdr) + "-matrix.txt"
            fdr_net_filename = fdr_prefix + "-" + str(i+1) + "-fdr-" + str(fdr) + "-network.txt"

            fdr_matr = gtm.save_gene_matrix(fdr_matr_filename,  matrix=acoefs_fdr[i], genes=genes)
            pickle.dump(threshes, open(fdr_prefix + "-" + str(i+1) + "-fdr-" + str(fdr) + "-threshes.p", 'wB'))
            ...
            sf_df = fc.summarize_fdr(matr=acoefs_fdr[i],  test=args.test_name,
                                    ...
                                    readme_name=fdr_prefix + "-" + str(i+1) + "-fdr-" + str(fdr) + "-README.txt",
        ...
        union_net_filename = fdr_prefix + "-union-fdr-" + str(fdr) + "-network.txt"
        union_net.to_csv(union_net_filename, sep="\t", index=False)
    ...
    all_sf_dfs.to_csv("fit_all_summary_fdr.txt", sep="\t", index=False)
    print "********"
    print "Summaries of all fdrs written to fit_all_summary_fdr.txt"
    print "Matrices done."
