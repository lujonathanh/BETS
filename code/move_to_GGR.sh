...
#topdir=/Users/jlu96/v-causal-snps/code/della/test-dir
topdir=/tigress/BEE/RNAseq/Data/Networks/GGR
...
while read runf; do
    ...
    mkdir $folder
    mkdir $folder/run
    mkdir $folder/run/$CAUSAL-$LAG
    mkdir $folder/networks
    echo "Moving " $runf "to" $folder/run/$CAUSAL-$LAG
    cp -r $runf/. $folder/run/$CAUSAL-$LAG
