...
def load_basic_rep_file_list(rep_file):
    """
    :param rep_file:
    :return: genes, geneTS
    not worrying about timekeys. assuming that each have the same shape
    """
    dfs = []

    dire = os.path.dirname(rep_file)
    ...
    with open(rep_file, 'rU') as csvfile:
        ...
        for row in reader:
            ...
            df = pd.read_csv(os.path.join(dire,row[0]), sep="\t")
...
def avg_and_std(df, keys=timekeys):
    ...
    :return: Updated version with a column of average, std
    """
